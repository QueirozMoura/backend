-- =============================================================
-- SISTEMA DE ASSISTÊNCIA TÉCNICA PROFISSIONAL
...
-- Tokens de refresh JWT (rotação de tokens)
...
CREATE TABLE permissions (
  ...
  name        VARCHAR(100) UNIQUE NOT NULL, -- ex: 'orders:create', 'clients:delete'
...
CREATE TABLE devices (
  ...
  category      VARCHAR(50)     NOT NULL, -- smartphone, tablet, notebook, desktop, tv, etc.
  brand_id      INTEGER         REFERENCES device_brands(id) ON DELETE SET NULL,
  brand_name    VARCHAR(100),  -- fallback se a marca não estiver no catálogo
  model         VARCHAR(150),
...
-- =============================================================
-- TÉCNICOS (PERFIL EXTENDIDO)
-- =============================================================

CREATE TABLE technicians (
  id              UUID         PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id         UUID         UNIQUE NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  specialties     TEXT[],                   -- ex: ARRAY['smartphone','notebook']
  commission_rate NUMERIC(5,2) DEFAULT 0,   -- percentual de comissão
...
CREATE TABLE parts (
  ...
  name        VARCHAR(200) NOT NULL,
  description TEXT,
  part_number VARCHAR(100),             -- código/SKU da peça
  category    VARCHAR(100),
  unit        VARCHAR(20)  DEFAULT 'unit', -- unit, meter, gram, etc.
...
-- Saldo atual de estoque por peça (tabela de posição)
CREATE TABLE stock (
  id               UUID          PRIMARY KEY DEFAULT uuid_generate_v4(),
  part_id          UUID          UNIQUE NOT NULL REFERENCES parts(id) ON DELETE RESTRICT,
  quantity         NUMERIC(10,3) NOT NULL DEFAULT 0,
  minimum_quantity NUMERIC(10,3) NOT NULL DEFAULT 0, -- alerta de estoque mínimo
  location         VARCHAR(100),                      -- prateleira/bin
  updated_at       TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);
...
CREATE TABLE stock_movements (
  ...
  reference_id   UUID,                 -- ID da OS ou pedido de compra relacionado
...
CREATE TABLE service_orders (
  id                       UUID                  PRIMARY KEY DEFAULT uuid_generate_v4(),
  order_number             VARCHAR(20)           UNIQUE NOT NULL, -- gerado automaticamente: OS-2024-00001
  client_id                UUID                  NOT NULL REFERENCES clients(id) ON DELETE RESTRICT,
...
-- Histórico completo de mudanças de status (audit trail da OS)
...
-- Fotos da OS (preparado para upload futuro)
...
CREATE TABLE audit_logs (
  ...
  entity_type VARCHAR(100) NOT NULL,   -- 'service_order', 'client', 'user', etc.
  entity_id   UUID,
...
CREATE INDEX idx_devices_active        ON devices(client_id) WHERE deleted_at IS NULL;

-- service_orders
CREATE INDEX idx_so_order_number  ON service_orders(order_number);
CREATE INDEX idx_so_client_id     ON service_orders(client_id);
CREATE INDEX idx_so_technician_id ON service_orders(technician_id);
CREATE INDEX idx_so_status        ON service_orders(status);
CREATE INDEX idx_so_created_at    ON service_orders(created_at DESC);
CREATE INDEX idx_so_active        ON service_orders(status) WHERE deleted_at IS NULL;

-- service_order_parts
CREATE INDEX idx_so_parts_order_id ON service_order_parts(service_order_id);
CREATE INDEX idx_so_parts_part_id  ON service_order_parts(part_id);
...
-- stock_movements
CREATE INDEX idx_stock_mov_part_id    ON stock_movements(part_id);
...
-- Sequência e geração automática de número da OS
...
-- Atualiza estoque automaticamente a partir de movimentações
CREATE OR REPLACE FUNCTION fn_update_stock_on_movement()
RETURNS TRIGGER AS $$
BEGIN
  -- Garante que a linha de estoque existe
...
-- Recalcula totais da OS quando peças são adicionadas/removidas
...
-- Usuário administrador padrão (senha deve ser atualizada pela aplicação)
-- A senha 'admin123' deve ser hasheada com bcrypt ANTES de usar em produção
INSERT INTO users (name, email, password_hash, role)
VALUES (
  'Administrador',
  'admin@assistencia.com',
  '$2b$10$CHANGE_THIS_HASH_IN_APP_BEFORE_USING',
  'admin'
...
INSERT INTO permissions (name, description) VALUES
  ...
  ('devices:create',      'Cadastrar aparelhos'),
  ('devices:read',        'Visualizar aparelhos'),
  ...
  ('orders:assign',       'Atribuir técnico a OS'),
  ('orders:status',       'Alterar status de OS'),
